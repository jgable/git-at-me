...
_.extend(GitAtMe.prototype, {
    ...
    start: function () {
        ...
        // If a hook is already created, no need to create it again
        if (this.config.skipHook) {
            self.server.ensureServer().then(function (server) {
                self.server.emit('server', server);
            });
        } else {
            // Otherwise, grab the token, then create the hook, then start the server
